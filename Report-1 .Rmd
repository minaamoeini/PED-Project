...
selected <- read_csv("/Users/mina/Desktop/AE project/Cleaned Dataset_Selected Fields.csv", col_types = cols(X1 = col_skip()))
sub_select <- selected %>% select ("record_id","shift_date", "shift_block", "ageyrs", "sex", "site", "los", 
                                   ...
                                   "pt_disposition", "AEoutcome","number_ae", "waiting_to_be_seen")
sub_select <- sub_select %>% mutate(yq = as.yearqtr(shift_date, format = "%Y-%m-%d"))

sub_select <- sub_select %>% group_by(site, shift_block, shift_date, ctas) %>% mutate(avg_waiting_to_be_seen_shift=mean(waiting_to_be_seen,na.rm = TRUE)) #Avg Los of each ctas level within each shift

sub_select$waiting_to_be_seen_cat <- "LOW"
sub_select$waiting_to_be_seen_cat[sub_select$los> mean(sub_select$waiting_to_be_seen, na.rm=TRUE)] <- "TRUE"
...
``` {r, warning=FALSE ,message=FALSE, echo=FALSE }
out <- table(sub_select$yq, sub_select$AEoutcome)  # shows number of AE outcome in each quarter and year
...
As it can be seen from the above table, the ratio of AE in each quarter is almost consistent. With the same appraoch we can find the ratio of **AEoutcome** in each **site**

**================ Precentage of AEoutcome in each site ================**
``` {r, warning=FALSE ,message=FALSE, echo=FALSE}
out <- table (sub_select$site, sub_select$AEoutcome) 
out <- cbind(out, Precentage = out[,2]/rowSums(out))
out
```
...
As it can be seen from the above table, the ratio of AE in each site is also consistent. We continue with investigating some of the feature that we have in the dataset. 


#### **1-1- Feature 1:Waiting to be seen**
...
ggplot(data=g, aes(x=AEoutcome, y=Ave_waiting_to_be_seen, fill=AEoutcome)) + 
  ...
  ylab("Average waiting to be seen")
```
...
With the same approach as we showed for waiting_to_be_seen, we can investigate age parameter as well. 

``` {r, warning=FALSE ,message=FALSE, echo=FALSE}
g <- sub_select %>% select(ageyrs, AEoutcome) %>% group_by(AEoutcome) %>% 
  summarise(Ave_age=round(mean(ageyrs, na.rm=TRUE), digit=2),n=n(), sd=sd(ageyrs,na.rm=TRUE), se=1.96*sd/sqrt(n))

ggplot(data=g, aes(x=AEoutcome, y=Ave_age, fill=AEoutcome)) + 
  geom_bar(stat="identity", position=position_dodge()) +
...
As it's shown, the average age for patients with positive AEoutcome is larger than patients with negative AEoutcome. We can also investigate this feature more in detail for each site as represented below:


``` {r, warning=FALSE ,message=FALSE, echo=FALSE}
g <- sub_select %>% select(site, ageyrs, AEoutcome) %>% group_by(site, AEoutcome) %>% 
  summarise(Ave_age=round(mean(ageyrs, na.rm=TRUE), digit=2),n=n(), sd=sd(ageyrs,na.rm=TRUE), se=1.96*sd/sqrt(n))
ggplot(data=g, aes(x=site, y=Ave_age, fill=AEoutcome)) +
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(aes(ymin=Ave_age-se, ymax=Ave_age+se), width=.2,position=position_dodge(.9))+
  scale_fill_manual(values=c("turquoise3", "orangered3"))+
  scale_x_continuous(breaks=c(0:8))+
...
``` {r, warning=FALSE ,message=FALSE, echo=FALSE}
g <- sub_select %>% select(ctas, ageyrs, AEoutcome) %>% group_by(ctas, AEoutcome) %>% 
  summarise(Ave_age=round(mean(ageyrs, na.rm=TRUE), digit=2),n=n(), sd=sd(ageyrs,na.rm=TRUE), se=1.96*sd/sqrt(n))

ggplot(data=g, aes(x=ctas, y=Ave_age, fill=AEoutcome)) +
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(aes(ymin=Ave_age-se, ymax=Ave_age+se), width=.2,position=position_dodge(.9))+
  scale_fill_manual(values=c("turquoise3", "orangered3"))+
  scale_x_continuous(breaks=c(0:8))+
...
ggplot(data=g, aes(x=site, y=Ave_waiting_to_be_seen, fill=AEoutcome)) + 
  ...
  scale_x_continuous(breaks=c(0:8))+
  labs(fill = "AEoutcome")+
  ylab("Average waiting time to be seen")
